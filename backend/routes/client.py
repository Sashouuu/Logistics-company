...
def update_client(client_id):
    """Update a client - only for employees or the client itself"""
    claims = get_jwt()
    user_id = claims.get("sub")
    ...
    # Only employees or the client can update
    if claims.get("role") != "EMPLOYEE" and client.user_id != user_id:
        return jsonify({"error": "Unauthorized"}), 403
...
def delete_client(client_id):
    """Delete a client - only for employees"""
    claims = get_jwt()
    if claims.get("role") != "EMPLOYEE":
        return jsonify({"error": "Unauthorized"}), 403
